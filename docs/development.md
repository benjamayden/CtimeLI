...
.venv/bin/pytest --cov=ctimeli/domain --cov-report=term-missing
```
...
grep -REn '^[[:space:]]*(import|from)[[:space:]].*(AppKit|objc|Cocoa|EventKit|ApplicationServices|adapters)' \
     src/ctimeli/domain src/ctimeli/app && echo "LAYERING VIOLATION" && exit 1

# Gate 2 — no bare user output outside adapters. Domain/app log via the port.
grep -REn '\bprint\(' src/ctimeli/domain src/ctimeli/app && echo "USE THE LOGGER" && exit 1
