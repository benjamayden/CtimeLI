...
def test_launch_permissions_relaunches_watch_from_embedded_terminal(monkeypatch, tmp_path):
    monkeypatch.setenv("CTIMELI_PERMISSIONS_MARKER", str(tmp_path / "setup"))
    relaunched: list[list[str]] = []
    monkeypatch.setattr(
        "ctimeli.adapters.macos.permissions.should_relaunch_permissions_in_terminal",
        lambda: True,
    )
    monkeypatch.setattr(
        "ctimeli.adapters.macos.permissions.relaunch_watch_in_terminal",
        lambda argv: relaunched.append(argv),
    )
    tidy = FakeWorkspaceTidy()
    cal = FakeCalendar(access=False)
    cfg = AppConfig(block_on_end=False, calendar_enabled=True)
    ok = request_watch_launch_permissions(
        cfg, logger=RecordingLogger(), workspace_tidy=tidy, calendar=cal, watch_argv=["--block-on-end"]
    )
    assert ok is False
    assert relaunched == [["--block-on-end"]]
...
def test_launch_permissions_marks_setup_only_when_granted(monkeypatch, tmp_path):
    monkeypatch.setenv("CTIMELI_PERMISSIONS_MARKER", str(tmp_path / "setup"))
    tidy = FakeWorkspaceTidy(access=False)
    cal = FakeCalendar(access=False)
    cfg = AppConfig(block_on_end=True, calendar_enabled=True)
    run_permissions_setup(
        cfg,
